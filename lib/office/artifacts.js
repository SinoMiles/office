...
export function artifactView(task, artifact) {
  ...
  return {
    ...
    artifactId,
    filename,
    fileType: artifact.fileType || extensionOf(filename),
    status: artifact.status || (task.status === 'completed' ? 'ready' : 'generating'),
    previewUrl: `/api/tasks/${taskId}/office-preview/proxy${routeId}/`,
    downloadUrl: `/api/tasks/${taskId}/download${artifactId ? `?artifactId=${encodeURIComponent(artifactId)}` : ''}`,
    live: true,
