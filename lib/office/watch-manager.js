...
function resolveLauncher() {
  const customDll = process.env.OFFICECLI_DLL;
  const customDotnet = process.env.OFFICECLI_DOTNET;
  if (customDll && customDotnet) {
    try {
      if (fs.existsSync(customDll) && fs.existsSync(customDotnet)) {
        return { command: customDotnet, prefixArgs: [customDll] };
      }
    } catch {
      // Custom development build is optional; fall through to packaged runtime.
    }
  }
