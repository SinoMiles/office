...
export default function ToolProcessPage() {
  ...
  const startProcessing = async () => {
    ...
    const toastId = toast.loading(t('tools.processingToast'));
    ...
    try {
      ...
      toast.success(t('tools.success'), { id: toastId });
    } catch (error) {
  ...
  return (
    <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '40px', textAlign: 'center', animation: 'slideUp 0.3s ease-out' }}>
        ...
        <h1 style={{ margin: '0 0 8px 0', fontSize: '2rem', fontWeight: 800 }}>{localizedName}</h1>
        <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '1.1rem' }}>{locale === 'zh-CN' ? tool.desc : seoContent.summary}</p>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: '800px', animation: 'slideUp 0.4s ease-out' }}>
        ...
        {tool.type === 'ai' ? (
          <div style={{ background: 'white', borderRadius: '24px', padding: '48px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.45rem', marginBottom: '12px' }}>{t('tools.aiStart')}</h2>
            ...
            <button onClick={() => router.push(`/dashboard?intent=${tool.id}`)} className="btn btn-primary" style={{ padding: '14px 28px', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              {t('tools.uploadStart')} <ArrowRight size={18} />
            </button>
          </div>
        ) : null}
          ...
          <div style={{ background: 'white', borderRadius: '24px', padding: '40px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)' }}>
              ...
              {files.length === 0 ? (
                <>
                  ...
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '8px' }}>{t('tools.drop')}</h3>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem' }}>{t('tools.formats', { formats: tool.accept })}</p>
                </>
              ) : (
                <div style={{ textAlign: 'left', maxWidth: '500px', margin: '0 auto' }}>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '16px' }}>{t('tools.filesReady', { count: files.length })}</h3>
                  ...
                  {tool.option ? (
                    <label style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px', fontSize: '0.9rem', fontWeight: 600 }}>
                      {locale === 'zh-CN' ? tool.option.label : localizedName}
                      <input type={tool.option.type || 'text'} autoComplete={tool.option.type === 'password' ? 'new-password' : undefined} value={optionValue} onChange={(event) => setOptionValue(event.target.value)} placeholder={locale === 'zh-CN' ? tool.option.placeholder : ''} style={{ width: '100%', padding: '12px 14px', border: '1px solid var(--border)', borderRadius: '10px', fontSize: '0.95rem', outline: 'none' }} />
                    </label>
                  ) : null}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    ...
                    >
                      {isProcessing ? (
                        <><Loader2 className="spin-anim" size={20} /> {t('tools.processing')}</>
                      ) : (
                        <><Sparkles size={20} /> {t('tools.start')}</>
                      )}
                    </button>
                  </div>
                </div>
        ...
        {tool.type !== 'ai' && resultUrl ? (
          <div style={{ background: 'white', borderRadius: '24px', padding: '60px 40px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)', textAlign: 'center', animation: 'slideUp 0.3s ease-out' }}>
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 700, marginBottom: '16px', color: 'var(--text-main)' }}>{t('tools.success')}</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '40px' }}>{t('tools.fileReady', { filename: resultFilename })}</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
              ...
              >
                <Download size={24} /> {t('tools.download')}
              </a>
      ...
      <section style={{ width: '100%', maxWidth: '900px', marginTop: '72px', display: 'grid', gap: '42px' }}>
        <div>
          <h2 style={{ fontSize: '1.65rem', marginBottom: '14px' }}>{t('tools.about', { name: localizedName })}</h2>
          <p style={{ color: 'var(--text-muted)', lineHeight: 1.85, fontSize: '1rem' }}>{seoContent.summary}</p>
        </div>
        <div>
          <h2 style={{ fontSize: '1.4rem', marginBottom: '18px' }}>{t('tools.useCases')}</h2>
        ...
        <div>
          <h2 style={{ fontSize: '1.4rem', marginBottom: '18px' }}>{t('tools.faq')}</h2>
        ...
        {relatedTools.length > 0 ? <div>
          <h2 style={{ fontSize: '1.4rem', marginBottom: '18px' }}>{t('tools.related')}</h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>{relatedTools.map((related) => <Link key={related.id} href={`/tools/${related.id}`} style={{ padding: '11px 15px', borderRadius: '10px', background: 'white', border: '1px solid var(--border)', color: 'var(--text-main)', textDecoration: 'none' }}>{localizedToolName(related, locale)}</Link>)}</div>
        </div> : null}
        <div style={{ padding: '30px', borderRadius: '20px', background: 'linear-gradient(135deg, #eef2ff, #ecfdf5)', border: '1px solid rgba(99,102,241,.15)', textAlign: 'center' }}>
          <h2 style={{ marginBottom: '10px' }}>{t('tools.continueTitle')}</h2>
          <p style={{ color: 'var(--text-muted)', marginBottom: '20px' }}>{t('tools.continueText')}</p>
          <button onClick={() => router.push('/dashboard')} className="btn btn-primary">{t('tools.openAi')}</button>
        </div>
      </section>
    </div>
  );
