...
export function getWechatPayConfig() {
  return {
    ...
    // 退款结果走独立回调地址；未单独配置时从支付回调地址推导，省一个必填项。
    refundNotifyUrl: process.env.WECHAT_PAY_REFUND_NOTIFY_URL
      || String(process.env.WECHAT_PAY_NOTIFY_URL || '').replace(/\/notify\/?$/, '/refund-notify'),
  };
}
...
// 退款金额与订单原始金额都必须提交，微信据此判断是全额还是部分退款。
export async function createWechatRefund({ outTradeNo, outRefundNo, refundFen, totalFen, reason }) {
  const config = getWechatPayConfig();
...
export async function queryWechatRefund(outRefundNo) {
  return wechatRequest('GET', `/v3/refund/domestic/refunds/${encodeURIComponent(outRefundNo)}`);
}
