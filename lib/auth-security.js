...
export async function createCaptcha() {
  const answer = Array.from({ length: 4 }, () => crypto.randomInt(10)).join('');
  ...
  const glyphs = [...answer].map((character, index) => `<text x="${34 + index * 42}" y="${42 + crypto.randomInt(-4, 5)}" transform="rotate(${crypto.randomInt(-15, 16)} ${34 + index * 42} 34)">${character}</text>`).join('');
...
export function phoneCodeHash(phone, purpose, code) {
  return secretHash(`${normalizePhone(phone)}:${purpose}:${code}`);
}

export async function consumePhoneCode({ phone, purpose, code }) {
  ...
  const actual = Buffer.from(phoneCodeHash(normalized, purpose, code));
