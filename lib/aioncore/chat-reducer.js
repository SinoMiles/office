...
export function mergeStreamMessages(messages, incoming) {
  ...
  if (['text', 'thought', 'thinking', 'content'].includes(incoming.type)) {
    ...
    const nextText = incoming.content?.content || (typeof incoming.data === 'string' ? incoming.data : incoming.data?.content) || '';
    merged = {
      ...previous,
      ...incoming,
...
export function mapMessagesToUi(messages, runtime) {
  ...
  for (const message of messages) {
    ...
    if (message.type === 'thought' || message.type === 'thinking') {
      const thought = typeof message.data === 'object' && message.data ? message.data : message.content || {};
      assistant.thought = {
        subject: thought.subject || '思考中',
        description: thought.description || thought.content || '',
      ...
      assistant.content = message.content?.content || message.content || (typeof message.data === 'string' ? message.data : message.data?.content) || '';
