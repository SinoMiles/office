...
const DEFAULT_ADMIN_PASSWORD = '34638339';
...
export async function seedDatabase() {
  try {
    ...
    if (existingAdmin) {
      const configuredPassword = process.env.ADMIN_PASSWORD?.trim() || DEFAULT_ADMIN_PASSWORD;
      if (!await bcrypt.compare(configuredPassword, existingAdmin.password)) {
        existingAdmin.password = configuredPassword;
        await existingAdmin.save();
        console.log(`Administrator credentials synchronized: ${adminEmail}`);
      }
    }
