...
export async function sendSubscriptionReminderEmail({ email, planName, daysLeft, currentPeriodEnd }) {
  ...
  return deliver({
    to: email,
    subject: `OfficeGPT · ${title}`,
    text: `您的 ${planName} 将于 ${endText} 到期。到期后账号将自动降级为免费版，每月赠送额度与会员权益将同时停止。`,
    html: layout({
      title,
      body: `<p>您的 <b>${planName}</b> 将于 <b>${endText}</b> 到期。</p><p style="color:#334155">到期后账号会自动降级为免费版，每月赠送额度与会员权益将同时停止。已充值的 Credits 余额不受影响，可继续使用。</p>`,
