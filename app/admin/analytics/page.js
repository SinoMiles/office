...
export default function AdminAnalyticsPage() {
  ...
  return <div style={{ animation: 'fadeIn .35s ease-out' }}>
    ...
    <section className="premium-stat-card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '850px', fontSize: '.84rem' }}><thead><tr style={{ background: 'var(--background)', textAlign: 'left' }}>{['创建时间','用户','订单号','金额','Credits','状态','支付时间'].map((title) => <th key={title} style={{ padding: '12px 16px', color: 'var(--text-muted)', fontWeight: 600 }}>{title}</th>)}</tr></thead><tbody>{(data?.payments || []).map((order) => <tr key={order.id} style={{ borderTop: '1px solid var(--border)' }}><td style={{ padding: '13px 16px' }}>{dateTime(order.createdAt)}</td><td style={{ padding: '13px 16px', fontWeight: 600 }}>{order.phone}</td><td style={{ padding: '13px 16px', color: 'var(--text-muted)', fontFamily: 'monospace' }}>{order.outTradeNo}</td><td style={{ padding: '13px 16px', fontWeight: 700 }}>¥{number(order.amount, 2)}</td><td style={{ padding: '13px 16px' }}>{number(order.credits, 2)}</td><td style={{ padding: '13px 16px' }}><span style={{ color: statusColor[order.status], background: `${statusColor[order.status]}14`, padding: '4px 8px', borderRadius: '8px' }}>{statusText[order.status] || order.status}</span></td><td style={{ padding: '13px 16px' }}>{dateTime(order.paidAt)}</td></tr>)}</tbody></table>{!loading && !data?.payments?.length && <div style={{ padding: '36px', textAlign: 'center', color: 'var(--text-muted)' }}>暂无充值订单</div>}</div>
    </section>
  </div>;
}
